...
/**
 * Navigasi admin seluler: tombol hamburger di header topbar membuka drawer
 * off-canvas dari kiri + backdrop gelap. Desktop memakai sidebar permanen,
 * sehingga komponen ini tersembunyi di `md:` ke atas.
 */
export function AdminDrawer({ menu }: { menu: NavGrup[] }) {
  const [buka, setBuka] = useState(false);
  const pathname = usePathname();

  const halamanAktif =
    menu.flatMap((g) => g.item).find((m) => jalurAktif(pathname, m.href))?.label ??
    "Ringkasan";
  ...
  return (
    <div className="contents md:hidden">
      ...
      <span className="truncate font-mono text-[11px] uppercase tracking-widest text-kertas-300">
        Admin / <span className="text-white">{halamanAktif}</span>
      </span>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto p-3">
          {menu.map((kelompok) => (
            <div key={kelompok.grup} className="mb-4 last:mb-0">
